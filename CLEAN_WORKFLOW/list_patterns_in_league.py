...
if len(sys.argv) < 2:
    print("Usage: python3 list_patterns_in_league.py <league>")
    sys.exit(1)
    ...
    for side in ["HOME", "AWAY"]:
        for interval in INTERVALS:
            ...
            if total > 0:
                ...
                print(f"{team} {side} {label} : {recurrence}% ({total_buts} buts sur {total} matches) - {buts_marques} marqués + {buts_encaisses} encaissés")
