#!/usr/bin/env python3
"""
Moniteur live pour championnat bulgare.
...
class BulgariaLiveMonitor:
    """Moniteur live pour matches bulgares."""
    
    # Équipes bulgares connues
    BULGARIAN_TEAMS = {
        "Arda", "Beroe", "Botev Plovdiv", "Botev Vratsa",
        "CSKA 1948 Sofia", "CSKA Sofia", "Cherno More", "Dobrudzha",
        "Levski Sofia", "Lok. Plovdiv", "Lokomotiv Sofia", "Ludogorets",
    ...
    def is_bulgarian_match(self, home_team: str, away_team: str) -> bool:
        """Vérifier si c'est un match bulgare."""
        ...
        for team in self.BULGARIAN_TEAMS:
    ...
    def normalize_team_name(self, team_name: str) -> str:
        ...
        for db_team in self.BULGARIAN_TEAMS:
            if db_team.lower() == team_norm.lower():
                return db_team
            # Correspondance partielle (ex: "Levski" -> "Levski Sofia")
    ...
    def analyze_match(self, match_info: Dict) -> Optional[Dict]:
        ...
        # Vérifier si bulgare
        if not self.is_bulgarian_match(home_team, away_team):
            logger.info(f"⏭️ Match non bulgare: {home_team} vs {away_team}")
            return None
        
        logger.info(f"🇧🇬 Match bulgare détecté: {home_team} vs {away_team} | Minute {minute}")
    ...
    def format_alert(self, analysis: Dict) -> str:
        ...
        lines = []
        lines.append("=" * 80)
        lines.append(f"🚨 ALERTE MATCH BULGARE - INTERVALLE CRITIQUE")
        lines.append("=" * 80)
                ...
                try:
                    ...
                    if self.should_alert(match['url'], minute):
                        ...
                        if self.telegram and self._should_send_telegram(analysis):
                            ...
                            self.telegram.send_alert(analysis, championship="Bulgarie")
    ...
    def monitor_continuous(self, duration_minutes: Optional[int] = None):
        ...
        """
        logger.info("🚀 Démarrage moniteur live Bulgarie")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Moniteur live matches bulgares')
    ...
    monitor = BulgariaLiveMonitor(db_path=args.db)
