...
class SoccerStatsLiveScraper:
    ...
    def extract_minute(self, soup: BeautifulSoup) -> Optional[int]:
        ...
        try:
            # 1) PRIORITÉ ABSOLUE: Chercher <font #87CEFA 13px> avec "XX min." (affichage live)
            ...
            # 3) FALLBACK: Détecter HT/FT seulement si aucune minute live trouvée
            html_text = soup.get_text()
            
            # Half-Time = 45'
            if re.search(r'\bHT\b|\bHalf[\s-]?Time\b', html_text, re.I):
                return 45
            
            # Full-Time = 90'
            if re.search(r'\bFT\b|\bFull[\s-]?Time\b', html_text, re.I):
                return 90
            
            # 4) Dernier fallback: première occurrence "XX min" dans la page
