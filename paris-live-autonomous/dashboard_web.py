...
class DashboardMonitor:
    ...
    def _scan_live_matches(self):
        """Scanne les matchs live (simulation pour test)"""
        # TODO: Implémenter le vrai scraping quand matchs disponibles
        # Pour l'instant, retourne des données de test
        matches = []
        
        # Exemple de match simulé
        if self.predictor and len(dashboard_state['live_matches']) == 0:
            # Premier scan: ajouter un match de test
            matches.append({
                'id': 'test_france_1',
                'league': 'france',
        ...
        elif len(dashboard_state['live_matches']) > 0:
            # Garder les matchs existants et mettre à jour
            for match in dashboard_state['live_matches']:
