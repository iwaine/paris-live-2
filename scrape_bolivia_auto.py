...
class BoliviaAutoScraper:
    ...
    def run(self):
        ...
        conn = sqlite3.connect(self.DB_PATH)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM soccerstats_scraped_matches WHERE league = 'bolivia'")
        conn.commit()
        conn.close()
        print("🧹 Anciennes données Bolivie supprimées de la base.")
        # Sauvegarder les nouveaux matches
        if all_matches:
